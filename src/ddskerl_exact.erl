...
-export([new/0, total/1, insert/2, merge/2, quantile/2]).
...
insert(#ddskerl_exact{data = Data, total = Total} = S, Val) when 0 < Val ->
    NewData = maps:update_with(Val, fun(X) -> X + 1 end, 1, Data),
    S#ddskerl_exact{data = NewData, total = Total + 1}.

?DOC("Merge two DDSketch instances.").
-spec merge(ddsketch(), ddsketch()) -> ddsketch().
merge(#ddskerl_exact{data = Data1, total = Total1}, #ddskerl_exact{data = Data2, total = Total2}) ->
    Data = maps:merge_with(fun(_K, Val1, Val2) -> Val1 + Val2 end, Data1, Data2),
    #ddskerl_exact{data = Data, total = Total1 + Total2}.
